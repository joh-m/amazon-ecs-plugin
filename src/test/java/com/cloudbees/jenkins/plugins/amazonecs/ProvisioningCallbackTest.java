...
public class ProvisioningCallbackTest {
    ...
    public void callbackCall_existingTemplate_runsEcsTask() throws Exception {
        ...
        Mockito.when(ecs.runEcsTask(any(ECSSlave.class), eq(template), anyString(), ArgumentMatchers.<Collection<String>>any(), eq(taskDefinition))).thenReturn("arn::aws::region::task");
    ...
    public void callbackCall_existingTaskDefinitionOverride_runsEcsTask() throws Exception {
        ...
        // should run task
        verify(ecs).runEcsTask(any(ECSSlave.class), eq(template), anyString(), ArgumentMatchers.<Collection<String>>any(), any(TaskDefinition.class));
    }
